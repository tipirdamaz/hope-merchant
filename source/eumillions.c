...
struct ListX2 *luckyStarBallStats = NULL;
 ...
 * (assign values to winningBallStats and luckyStarBallStats global variables)
 * 
 */
 ...
 * @param {struct ListX *} drawnBalls     : refers to balls drawn in a new draw. 
 * @param {Integer} drawBallCount     	  : DRAW_BALL for normal ball list or DRAW_BALL_LS for luckyStar list (coupon column count)
 * @return {Integer}                      : Returns 1 if found, 0 if not.
 */
...
UINT8 search2CombX(struct ListX *aPrvDrawn, struct ListX *drawnBalls, struct ListXY *luckyBalls, char *buf)
{
	...
	if (drawnBalls->size == 2 && aPrvDrawn->size == 2) 
	{
		if (seqSearchX2(aPrvDrawn, b1) >= 0)
		{
			if (buf) {
			...
			if (luckyBalls) {
				if ((index = seqSearchXY2(luckyBalls, b1)) < 0) {
...
UINT8 init()
{
	...
	#ifndef __MSDOS__
	if (!(fileStatRows = getDrawnBallsList(luckyStarDrawnBallsList, fileStats))) {
		printf("%s file or record not found!\n", fileStats);
		return 0;
	}
	#endif
	...
	luckyStarBallStats = createListX2(luckyStarBallStats);
	getDrawnBallsStats();
...
struct ListXY * getLuckyStars(struct ListXY *luckyStars)
{
	...
	{
		aPrvDrawn1 = getListXByIndex(luckyStarDrawnBallsList, i);

		for (j=i+1; j<fileStatRows; j++)
		{
			aPrvDrawn2 = getListXByIndex(luckyStarDrawnBallsList, j);
...
struct ListXY * getLuckyStarsFromFile(struct ListXY *luckyStars)
{
	...
	for (i=0; fgets(ioBuf, 20, fp) && i<UINT16MAX; i++)
	{
		...
		drawList = createListX(drawList, 2, NULL, (UINT16) val, 0, 0, 0);
...
void getDrawnBallsStats()
{
	...
	for (i=0; i<TOTAL_BALL_LS; i++) {
		appendItem2(luckyStarBallStats, i+1, 0);
	}
	...
	for (j=0; j<fileStatRows; j++) 
	{
		...
		if ((index = seqSearch2X1(luckyStarBallStats,ls1)) != -1) incVal2(luckyStarBallStats, index);
		if ((index = seqSearch2X1(luckyStarBallStats,ls2)) != -1) incVal2(luckyStarBallStats, index);
	}
}
...
mainMenu:
	...
	bubbleSortX2ByVal(luckyStarBallStats, -1);

	puts("EuroMillions Lotto 1.0 Copyright ibrahim Tipirdamaz (c) 2022\n");
	...
	printBallStats(luckyStarBallStats);
	printf("\n\n");
...
exitProgram:
	...
	removeAllX2(luckyStarBallStats);
