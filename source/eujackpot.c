...
struct ListXY * getEuroNumbers(struct ListXY *euroNumbers)
{
	...
	for (i=0; i<euNumberBallRows; i++)
	{
		aPrvDrawn1 = getListXByIndex(euNumberDrawnBallsList, i);

		for (j=i+1; j<euNumberBallRows; j++)
		{
			aPrvDrawn2 = getListXByIndex(euNumberDrawnBallsList, j);
...
void getDrawnBallCount()
{
	...
	for (j=0; j<euNumberBallRows; j++) 
	{
		...
		if ((index = seqSearchX2(euNumberBallsDrawCount,ls1)) != -1) incVal2(euNumberBallsDrawCount, index);
		if ((index = seqSearchX2(euNumberBallsDrawCount,ls2)) != -1) incVal2(euNumberBallsDrawCount, index);
	}
}
